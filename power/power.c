...
static void x3_power_init( __attribute__((unused)) struct power_module *module)
{
    ...
    sysfs_write("/sys/devices/system/cpu/cpufreq/interactive/boost_factor",
				"0");
    sysfs_write("/sys/module/cpu_input_boost/parameters/enabled",
				"1");
    uevent_init();
}

static void x3_power_set_interactive(__attribute__((unused)) struct power_module *module,
                                          __attribute__((unused)) int on)
{
	if (on) {
		sysfs_write("/sys/devices/system/cpu/cpufreq/interactive/go_maxspeed_load", "75");
		sysfs_write("/sys/module/cpu_input_boost/parameters/enabled", "1");
		sysfs_write(MAX_FREQ_PATH, NORMAL_MAX_FREQ);
	}
	else {
		sysfs_write("/sys/devices/system/cpu/cpufreq/interactive/go_maxspeed_load", "85");
		sysfs_write(MAX_FREQ_PATH, LOW_POWER_MAX_FREQ);
	}
}

static void cpu_boost(struct power_module *module, void *data)
{
	int duration = 2000;
	int min_freq, cpu;

	ALOGD("%s: CPU boost duration: %d", __func__, duration);

    sysfs_write(MIN_FREQ_PATH, BOOST_FREQ);
    usleep(duration * 1000);
    sysfs_write(MIN_FREQ_PATH, LOW_POWER_MIN_FREQ);
}
	

static void x3_power_hint(__attribute__((unused)) struct power_module *module, power_hint_t hint,
                            void *data)
{
    char buf[80];
    int len, cpu, ret;
    ...
    case POWER_HINT_CPU_BOOST:
        ALOGD("%s: POWER_HINT_CPU_BOOST", __func__);
        cpu_boost(module, data);
        break;
    ...
    case POWER_HINT_LOW_POWER:
        pthread_mutex_lock(&low_power_mode_lock);
        if (data) {
			ALOGD("%s: POWER_HINT_LOW_POWER: true", __func__);
            low_power_mode = true;
			...
			ALOGD("%s: POWER_HINT_LOW_POWER: false", __func__);
    ...
    case POWER_HINT_INTERACTION:
        ALOGV("%s: POWER_HINT_INTERACTION handled by kernel", __func__); //TODO: Add kernel interface
        break;
    default:
            break;
    }
...
struct power_module HAL_MODULE_INFO_SYM = {
    ...
    .init = x3_power_init,
    .setInteractive = x3_power_set_interactive,
    .powerHint = x3_power_hint,
};
